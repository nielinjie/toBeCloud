 ...
 * @author Philipp Haller
 */
class TcpService(port: Int, cl: ClassLoader) extends Thread with Service with Logger{
  ...
  logger.debug(LocalAddress.getFirstNonLoopbackAddress(true,false).toString())
  private val internalNode = new Node(LocalAddress.getFirstNonLoopbackAddress(true,false).getHostAddress(), port)
